...
public class CircularLinkedList<T> {
	...
	public Node<T> getNodeByNormalizedIndex(int index) throws IndexOutOfBoundsException {
	...
	public boolean insertNode(int key, T value) throws IllegalArgumentException {
		if (key < 1 || key > 1000000)
			throw new IllegalArgumentException("Key must be in range from 1 to 10^6(inclusive)");
	...
	private List<Node<T>> findNodesByKey(int key) {
		...
		while (prevNearestNodePathLength < count && prevNearestNode.getKey() != key) {
			prevNearestNode = prevNearestNode.getPrev();
			++prevNearestNodePathLength;
		}
		if (prevNearestNodePathLength == count)
			return null;

		while (nextNearestNodePathLength < count - prevNearestNodePathLength && nextNearestNode.getKey() != key) {
			nextNearestNode = nextNearestNode.getNext();
			++nextNearestNodePathLength;
		}
		if (prevNearestNodePathLength == (count - nextNearestNodePathLength) % count)
			return Arrays.asList(prevNearestNode);
		if (prevNearestNodePathLength == nextNearestNodePathLength)
			return Arrays.asList(prevNearestNode, nextNearestNode);
		if (prevNearestNodePathLength < nextNearestNodePathLength)
			return Arrays.asList(prevNearestNode);
		else
			return Arrays.asList(nextNearestNode);
	}
